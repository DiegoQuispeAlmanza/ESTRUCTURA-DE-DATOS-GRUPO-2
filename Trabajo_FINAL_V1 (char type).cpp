...
struct Proceso {
    ...
    Proceso* siguiente;		//Genera una conexion al siguiente nodo
};
...
Proceso* head = NULL;		//Inicializar el Puntero Proceso a head
...
//FUNCION CONTADOR DE ELEMENTOS DE LA LISTA
int contadorID(Proceso*& head){
	
	int cont_lista = 0;		//Iniciar en 0
	
	if (head == NULL){		//Si la cabez se halla vacia (head = NULL)
		cout << "\nNo se encontro la cabeza en la lista" << endl;
		return cont_lista;	//Regresa el valor del contador en 0
		
	}else{
		cont_lista++;		//Como ya existe una cabeza en la lista, se cuenta el elemento 
		
		Proceso* aux = head;	//Puntero auxiliar (aux) a la cabeza (head)
		
		while(aux->siguiente != NULL){		//Recorre los nodos existentes, hasta dar con un nodo que se encuentre vacio, es decir, NULL
			cont_lista++;			//Cuenta el nodo existente
			aux = aux->siguiente;	//Pasa al siguiente nodo dentro de la lista
		}
		
		cout << "\nLa cantidad de elementos en la lista es: " << cont_lista << endl;		//Texto para ver el valor del contador
		return cont_lista;		//Regresa el valor contado de los nodos existentes en la lista
	}
}
//===============================================================================
//===============================================================================

//Funcion para reordenar los ID de los elementos de la lista
void reordenarProcesos(Proceso*& head){
	
	int comp = contadorID(head);	//Realiza funcion contador para conseguir el valor
	
	if (head == NULL){		//Verfica si existe la cabeza
		cout << "\nLa cabeza esta vacia en la lista" << endl;
		
	}else{
		Proceso* aux = head;	//Puntero auxiliar (aux) a la cabeza
		
		for(int i = 0; i < comp; i++){	//Como cuenta los nodos existentes, este valor se usa en un ciclo for para reorganizar la lista de inicio a fin, y asignar sus valores
			
			if(aux->siguiente != NULL){	//Asigna los valores existentes hasta dar con un nodo vacio, es decir, NULL
				aux->id = i;
				aux = aux->siguiente;	
			}else{
				break;	//Si da con un NULL, se termina el proceso
			}						
		}
		
		cout << "Se reorganizo la lista actual " << endl; //Mostrar texto de proceso realizado
	}
}
//===============================================================================
...
void insertarProceso(Proceso*& head) {
    ...
    cout << "Nombre: ";
    cin.getline(nuevoNodo->nombre, 50);
...
void encolarProceso(Proceso*& head, NodoCola*& frente, NodoCola*& final) {
    ...
    while (aux != NULL) {
        ...
        strncpy(nuevoCola->nombre_cola, aux->nombre, sizeof(nuevoCola->nombre_cola));
...
void cargarMemoriaDesideCola(NodoCola*& frente) {
    ...
    while (aux_cola != NULL && count < MAX) {
        ...
        strncpy(nuevoPila->nombre_pila, aux_cola->nombre_cola, sizeof(nuevoPila->nombre_pila));
        
        nuevoPila->memoria_pila = ram_encontrada;
        nuevoPila->siguiente_pila = NULL;

        temp[count] = nuevoPila;
        count++;
...
void liberarMemoria(NodoPila*& tope) {
...
void guardarDatos() {
    ...
    ofstream archivo("procesos_FINAL.txt");
...
void menuMemoria(int opMenu) {
    ...
    do {
        ...
        switch (opMenu) {
            case 1: cargarMemoriaDesideCola(frente); 	break;
            case 2: liberarMemoria(tope);         		break;
